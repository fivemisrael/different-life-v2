...
function IsMenuSectionExist(name)
	for k,v in pairs(Menu.GUI) do
		if v.name == name then
			return true
		end
	end
...
function IsSelectedMenuActive(name)
	for k,v in pairs(Menu.GUI) do
		if v.name == name and v.active then
			return true
		end
	end
	return false
...
RegisterCommand("namec", function()
	print(json.encode(Menu.GUI))
end)
