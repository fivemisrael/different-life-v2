var InventoryOption = "235, 235, 235, 0.05";
...
$(document).on("dblclick", ".item-slot", function(e){
    e.preventDefault();
    if ($(this).data("item") != null) {
        var fromData = $(this).data("item");
        var fromInventory = $(this).parent().data("inventory");
        if (fromData.useable){
            if(fromData.shouldClose){
                Inventory.Close();
            }
            $.post("http://dl-inventory/UseItem", JSON.stringify({
...
function FormatAttachmentInfo(data) {
    $.post("http://dl-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('http://dl-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('http://dl-inventory/Notify', JSON.stringify({
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            if (itemData.info.type == "plate") {
                $(".item-info-description").html('<p><strong>Stolen Plate Number: </strong><span>' + itemData.info.plate + '</span></p><p><strong>Vehicle Source Name: </strong><span>' + itemData.info.vehiclesourcename + '</span></p>');
            }
...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        start: function(event, ui) {
           ...
           // $(this).css("background", "rgba(20,20,20,1.0)");
            ...
            if (!itemData.useable) {
                $("#item-use").css("background", "rgba(255, 255, 255, 0.01");
            }
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://dl-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://dl-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://dl-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://dl-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("http://dl-inventory/PlayDropSound", JSON.stringify({}));
            $.post("http://dl-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('http://dl-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("http://dl-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("http://dl-inventory/CloseInventory", JSON.stringify({}));
        ...
        window.addEventListener('message', function(event) {
            switch(event.data.action) {
                ...
                case "toggleHotbar":
                    ...
                    break;
                case "RobMoney":
                    $(".inv-options-list").append('<div class="inv-option-item" id="rob-money"><p>ROB MONEY</p></div>');
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('http://dl-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});