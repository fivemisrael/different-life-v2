...
local toggle_spotlight = 311 -- control id to toggle the front spotlight Default: INPUT_PhoneCameraGrid (G)
local toggle_lock_on = 22 -- control id to lock onto a vehicle with the camera. Default is INPUT_SPRINT (spacebar)

-- Script starts here
local helicam = false
local maverick2_hash = GetHashKey("maverick2")
...
function IsPlayerInmaverick2()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, maverick2_hash) or IsVehicleModel(vehicle, polmav_hash)
end
