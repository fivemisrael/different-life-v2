...
document.onreadystatechange = () => {
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            ...
            } else if (event.data.action == "open-complaint") {
                lastData.complaint = true;
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            Databank.Close();
            Fingerprint.Close();
            if (lastData.complaint) {
                $(".complaint-container").slideUp();
                $.post('https://police/close-from',JSON.stringify({}))
            }
            break;
    }
});
...
function complaint(){
    $(".btn-grid").on("click", function(){
        ...
        if(!retval && $(".my-form textarea").value != ""){
            ...
            $.post('https://police/send',JSON.stringify({
                data : inputVal
            }));
            restartInputs();
        }
